...
export function ProgressEvidence({job}:{job:any}) {
 ...
 <details><summary>阶段执行记录（{job.attempts?.length||0}）</summary>{job.attempts?.map((a:any,i:number)=><p key={i}>{a.role||a.stage} · {attemptLabel(a)} · 调用 #{a.attempt}{a.format_repair_of&&` · 原响应格式修复 ${a.format_repair_attempt}/2`}</p>)}</details>
